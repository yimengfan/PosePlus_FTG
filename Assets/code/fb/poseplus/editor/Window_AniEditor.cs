...
class Window_AniEditor : EditorWindow
{
    ...
    public void OnGUI()
    {
        ...
        GUILayout.EndHorizontal();
        string[]  toolsStr = new string[]{"动作编辑","碰撞盒编辑","触发点编辑","特效编辑","声音编辑"};
        ...
        else if(mToolsBar == 3)  //特效编辑模式
        {
            effectpos = GUILayout.BeginScrollView(effectpos, GUILayout.Width(300));
            {
                GUI_Effect();
            }
            GUILayout.EndScrollView();
...
//                     }
    ...
    void SaveEditData()
    {
        ...
        //foreach (Transform o in aniPlayer.transform.FindChild("_dotes"))
        //{
    ...
    void GUI_Effect()
    {
        ...
        GUILayout.Space(5);
        effectName = GUILayout.TextField(effectName, GUILayout.Width(80));
        GUILayout.Label("生命周期",GUILayout.Width(50));
        var str = GUILayout.TextField(effectlife.ToString(), GUILayout.Width(50));
      
        isFollow= GUILayout.Toggle(isFollow,"跟随");
        uint.TryParse(str, out effectlife);
        GUILayout.EndHorizontal();
        if (GUILayout.Button("载入特效"))
        {
        ...
        effectScroll = GUILayout.BeginScrollView(effectScroll, GUILayout.Width(300));
        GUILayout.BeginHorizontal();
    ...
    void GUI_Audio()
    {
        ...
        for (int i = editor.aniInEdit.frames[curframe].aduioList.Count-1; i >= 0; i--)
        {
            ...
            if(GUILayout.Button("Play",GUILayout.Width(45)))
            {
                ...
                //aniPlayer.iextplayer.PlaySound();
            }
